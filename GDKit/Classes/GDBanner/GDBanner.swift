...
protocol GDBannerDelegate: AnyObject {
    func bannerNumbers() -> Int
    func bannerView(banner: GDBanner, index: Int, indexPath: IndexPath) -> UICollectionViewCell
    func didSelected(banner: GDBanner, index: Int, indexPath: IndexPath)
    func didStartScroll(banner: GDBanner, index: Int, indexPath: IndexPath)
    func didEndScroll(banner: GDBanner, index: Int, indexPath: IndexPath)
}

protocol GDBannerPageControl where Self: UIView {
    /// 当前下标
...
class GDBanner: UIView {
    //MARK: - 构造方法
    init(frame: CGRect, flowLayout: GDSwiftFlowLayout, delegate: GDBannerDelegate) {
    ...
    required init?(coder aDecoder: NSCoder) {
        self.flowLayout = GDSwiftFlowLayout.init(style: .unknown)
        super.init(coder: aDecoder)
    }
    
    //MARK: - Property
    /// 自定义layout
    let flowLayout: GDSwiftFlowLayout
    /// collectionView
...
//        let rect = self.bounds
    ...
    /// 外部代理委托
    weak var delegate: GDBannerDelegate?
    /// 当前居中展示的cell的下标
    ...
    lazy var pageControl: UIPageControl = {
        ...
        pageControl.currentPageIndicatorTintColor = UIColor.gd_get("E76129")
        pageControl.translatesAutoresizingMaskIntoConstraints = false;
        return pageControl
    }()
    /// 自定义的pageControl
    var customPageControl: GDBannerPageControl? {
        willSet
        {
...
extension GDBanner: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
